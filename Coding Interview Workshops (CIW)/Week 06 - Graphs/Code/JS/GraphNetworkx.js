var jsnx = require('jsnetworkx');

// Create a new graph
var eg_graph = new jsnx.Graph();

// Add a node to the graph
eg_graph.addNode("a");

// Add a list of nodes to the graph
nodes = ["b", "c", "d", "e"];
//eg_graph.addEdgesFrom(nodes);

// // You can also add node atrributes to each node using the form:
// // ("b", {colour: "blue"})

// Add an edge to the node
eg_graph.addEdge("a", "b");

// Edges can be added after edge creation or at the same time
eg_graph.addEdge("a", "c", {weight: 4});
eg_graph.get("a").get("b").weight = 10;

// Multiple edges and weights can be added at once
eg_graph.addEdgesFrom([["b", "d", {"weight": 3}], 
                         ["d", "c", {"weight": 7}], 
                         ["e", "d", {"weight": 2}]]);

// We can visualise the Graph like this
// jsnx.draw(eg_graph, );

// To create a directed graph, we use a different constructor
var eg_digraph = new jsnx.DiGraph();

// Adding nodes and edges can be done the same way
// We can also import the edges and nodes from another graph
eg_digraph.addNodesFrom(eg_graph);
eg_digraph.addEdgesFrom(eg_graph.edges);

// Let's visualise the graph
// jsnx.draw(eg_digraph, with_labels=True, font_weight='bold');